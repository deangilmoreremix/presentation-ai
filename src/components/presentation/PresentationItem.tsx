...
} from "@/app/_actions/notebook/presentation/presentationFavoriteActions";
...
export function PresentationItem({
  ...
  const { mutate: favoriteMutation, isPending: isFavoritePending } =
    useMutation({
      mutationFn: async () => {
        if (isFavorited) {
          return removePresentationFromFavorites(presentation.id);
        }
        return addPresentationToFavorites(presentation.id);
      },
