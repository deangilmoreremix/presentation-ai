-- Migration 0001: Initial schema for the ALLWEONE Presentation app.
-- Self-contained and IDEMPOTENT: uses CREATE TABLE IF NOT EXISTS /
-- CREATE INDEX IF NOT EXISTS so it can be re-applied to a fresh database
-- (or re-run on an already-migrated one) without errors.
-- Columns are quoted camelCase to match the Prisma field names the app expects.
-- RLS policies live in a separate 0002 migration.
